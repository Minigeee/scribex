...
export async function startQuest(questId: string) {
  try {
    ...
    // Check if the quest has stat prerequisites
    const statPrerequisites =
      (quest.prerequisite_stats as Record<string, number>) || {};

    // Convert the prerequisite_stats object to an array of StatPrerequisite objects
    const statPrerequisiteArray = Object.entries(statPrerequisites).map(
      ([stat, value]) => ({ stat, value })
    );

    if (statPrerequisiteArray.length > 0 && characterProfile) {
      const characterStats =
        (characterProfile.stats as Record<string, number>) || {};

      // Check if the character meets all stat prerequisites
      for (const prerequisite of statPrerequisiteArray) {
        const characterStatValue = characterStats[prerequisite.stat] || 0;
        if (characterStatValue < prerequisite.value) {
          return {
            error: 'PREREQUISITES_NOT_MET',
            message: `You need ${prerequisite.stat} of at least ${prerequisite.value} to start this quest.`,
            prerequisites: statPrerequisiteArray,
