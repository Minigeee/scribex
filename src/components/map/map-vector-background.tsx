  ...
  <g key={key}>
    ...
    {Array.from({ length: 6 }).map((_, i) => {
      ...
      return (
        <g
          key={`${key}-frond-${i}`}
          transform={`translate(${x + size / 2}, ${y - size / 2}) rotate(${(angle * 180) / Math.PI})`}
        >
          <ellipse
            cx={0}
            cy={0}
  ...
  const svgContent = useMemo(() => {
        ...
        if (view === 'biomes') {
          ...
          switch (center.biome) {
            ...
            case 'SUBTROPICAL_DESERT':
              ...
              break;
            default:
              fill = '#d7cba8'; // Default land color
          }
        } else if (view === 'elevation') {
          // Gradient from blue (low) to white (high)
          const elevationColor = Math.floor(
            Math.min(255, Math.max(0, center.elevation * 255))
          )
            .toString(16)
            .padStart(2, '0');
          ...
          const moistureColor = Math.floor(
            Math.min(255, Math.max(0, center.moisture * 255))
          )
            .toString(16)
            .padStart(2, '0');
    ...
    const borderLines = edges.map((edge, index) => {
      ...
      const isLandWaterBorder =
        (center0.water && !center1.water) || (!center0.water && center1.water);
        ...
        .map((center, index) => {
          ...
          switch (center.biome) {
            ...
            case 'BEACH': {
              ...
              const hills = [
                renderHill(featureX, featureY, hillSize, `hill-${index}`),
              ];
    ...
    const parchmentTexture =
      view === 'stylized' && renderParchmentTexture(width, height);
