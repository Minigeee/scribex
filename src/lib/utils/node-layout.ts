...
export const getLayoutedElements = (
  ...
  while (
    reverseAdjacencyList[currentNode] &&
    reverseAdjacencyList[currentNode].length > 0
  ) {
  ...
  const identifyBranches = () => {
    ...
    const branchPoints = Array.from(mainBranchNodes).filter((nodeId) =>
    ...
    if (nodeInBranch[node.id] === undefined) {
      ...
      while (queue.length > 0) {
        ...
        if (nodeInBranch[currentId] !== undefined) {
          const currentBranch = nodeInBranch[currentId];
  ...
  const positionedEdges = edges.map((edge) => {
    ...
    const isMainBranchEdge =
      sourceNodeBranchIndex === 0 || targetNodeBranchIndex === 0;
