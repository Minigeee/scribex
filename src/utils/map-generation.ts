...
export const getCorner = (corners: Corner[], x: number, y: number): number => {
  for (let i = 0; i < corners.length; i++) {
    if (
      Math.abs(corners[i].point.x - x) < 0.01 &&
      Math.abs(corners[i].point.y - y) < 0.01
    ) {
      return i;
    }
  }
...
export const getEdge = (
  ...
  for (let i = 0; i < edges.length; i++) {
    ...
    if (
      (v0 === cornerA && v1 === cornerB) ||
      (v0 === cornerB && v1 === cornerA)
    ) {
      if (
        (d0 === centerA && d1 === centerB) ||
        (d0 === centerB && d1 === centerA)
      ) {
        return i;
      }
    }
...
export const assignClusters = (
  ...
  if (
    ...
    center.ocean ||
    center.border ||
    center.distanceFromCenter === undefined
  )
    return;
...
export const generateMap = (
  config: MapConfig
): { centers: Center[]; edges: Edge[]; corners: Corner[] } => {
  ...
  const relaxedPoints = performRelaxation(
    points,
    config.width,
    config.height,
    config.relaxationIterations
  );
  ...
  assignElevation(centers, corners);
...
const assignOceanAndLand = (centers: Center[], config: MapConfig): void => {
  ...
  if (config.islandShape === 'radial' || !config.islandShape) {
    ...
    centers.forEach((center) => {
      ...
      let radialFactor =
        1 - distanceFromCenter / (maxPossibleDistance / config.islandFactor);
      ...
      let noiseValue = noise.fBm(
        x / (config.width / noiseScale),
        y / (config.height / noiseScale),
        4
      );
      ...
      const adjustedFactor =
        radialFactor + (noiseValue - 0.5) * noiseIntensity * 2;
...
const generateRivers = (
  ...
  for (let i = 0; i < riverCount; i++) {
    ...
    while (!terminated) {
      ...
      const riverEdge = current.protrudes.find(
        (edge) =>
