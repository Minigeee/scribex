 ...
 */
export const drawMountain = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number) => {
  // Draw mountain
 ...
 */
export const drawEvergreenTree = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number) => {
  // Draw trunk
 ...
 */
export const drawDeciduousTree = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number) => {
  // Draw trunk
  ctx.fillStyle = '#8B4513'; // Brown trunk
  ctx.fillRect(x - size/8, y, size/4, size/2);
 ...
 */
export const drawTropicalTree = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number) => {
  // Draw curved trunk
 ...
 * Draw a hill
 */
export const drawHill = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number) => {
  // Draw a curved hill
 ...
 */
export const drawSwamp = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number) => {
  // Base circle (water)
  ...
  for (let i = 0; i < 5; i++) {
    ...
    ctx.beginPath();
    ctx.ellipse(reedX, reedY - size/2, size/6, size/12, 0, 0, Math.PI * 2);
    ctx.fill();
  }
};
 ...
 */
export const addParchmentTexture = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
...
export const renderMap = (
  ...
  if (view === 'stylized') {
    ...
    centers.forEach(center => {
      ...
      let color = center.ocean ? '#1a6ca0' : // Deep blue for ocean
                  center.water ? '#4A7CDF' : // Lighter blue for lakes
    ...
    edges.forEach(edge => {
      if (edge.d0 && edge.d1) { // Make sure the edge has centers on both sides
